...
_WELCOME_MESSAGE = """
Welcome to the Blind Auction for the Diamond!\n
                        '
               '                 '
       '         '      '      '        '
          '        \    '    /       '
...
def update_bids(name: str, bids: Dict[str, float]) -> Dict[str, float]:
    """
    Update bids for a participant.
    Specifically, ask client if they want to keep or add their bids. If they want to add bids,
    update the new bidding price in the dictionary.
    Adjustment limit: cannot add more than 50% of their original bidding price.
    ...
    Returns:
        dict: the updated dictionary with new values(prices).
    """
    while True:
